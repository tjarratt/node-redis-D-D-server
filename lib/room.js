function initRoom() {
  ...
  function message(obj){
    ...
    if ("announcement" in obj) {
      ...
      if (obj.username && obj.imageName) {
        ...
        newUserBlock += "<div id='" + username + "'><div class='innerChat'><a href='#' class='chatLink' id='" + username + "'></div></div></div>";
  ...
  $("a.chatLink").each(function(index, chatlink) {
    jQuery("a#" + this.id).click(function(e) {
      ...
      var initiatedChat = function(transport) {
        ...
        var chatSocket = new io.Socket(_url, {port: 8080});
  ...
  _draggingImage = null;
  if (_isDM) {
    _dmAction = "annotate"; //TODO: store this in local storage?
     var colorWheelContext = $("canvas#annotateColor")[0].getContext("2d");
     var colorWheelImage = new Image;
     colorWheelImage.src = "/res/img/colorwheel.png";
     colorWheelImage.onload = function() {
       colorWheelContext.drawImage(colorWheelImage, 0, 0);
     }
  ...
  _$textArea.mouseover(function(e) {
    ...
    if (!e.fromElement || !e.fromElement.src || jQuery(e.fromElement).parent().attr("class") != "playerBlock") {return false;}
