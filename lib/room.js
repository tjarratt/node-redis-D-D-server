function initRoom() {
  ...
  function handleMovement(aMove) {
    ...
    var src = $("img#player" + moveInfo.from);
    if (!src || src.length == 0) {
    ...
    image.onload = function() {
      ...
      _positions[key] = {source : src.attr('src'), coords: coords};
  ...
  function message(obj){
    ...
    if ("announcement" in obj) {
      ...
      if (obj.username && obj.imageName) {
        $("div#players").append("<img id='player" + esc(obj.username) + "' src='/res/img/" + esc(obj.imageName) + ".png' />");
      }
    }
